...
TpGuile* TpGuile::instance = NULL;

TpGuile* TpGuile::getImplemention(){
  if(instance == NULL)
    instance = new TpGuile();
  return instance;
}
...
TpGuile::TpGuile() {
    std::string  designTypeDefFile = "t1";

    scm_init_guile();
    scm_c_primitive_load( designTypeDefFile.c_str());
}
