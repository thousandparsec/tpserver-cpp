...
void TlsSocket::process(){
    Logger::getLogger()->info("Accepting new secure connection");
    try{
#ifdef HAVE_IPV6
        struct sockaddr_storage clientaddr;
        socklen_t addrlen = sizeof(clientaddr);

        Connection *temp = new PlayerTlsConnection(accept(sockfd, 
        ...
        Logger::getLogger()->info("Secure connection accepted from %s [%s], connection id %d", 
                                    clientname, clienthost, temp->getFD());
        
#else
        
        Connection *temp = new PlayerTlsConnection(accept(serverFD, NULL, 0));
        Network::getNetwork()->addConnection(temp);
#endif
    }catch(std::exception e){
        Logger::getLogger()->warning("Could not establish secured connection");
    }
